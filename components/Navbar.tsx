...
export default function Navbar() {
    ...
    const navItems = (href : string, label: string) => {
        ...
        return(
            <Link href={href} className={`text-xs md:text-base flex items-center gap-1 hover:uppercase hover:scale-110 hover:tracking-wider transition-all ${isActive ? "scale-110 text-fuchsia-500 tracking-wider" : ""}`}>
    ...
    return(
        <div className={`flex border-b border-zinc-100/90 mb-4 w-full z-50 bg-transparent top-0 uppercase p-4 justify-end gap-8 sticky bg-white/80 backdrop-blur-sm ${press.className}`}>
            ...
            {navItems("/projects", "Project")}
            {navItems("/aboutme", "Contact me")}
        </div>
    )
